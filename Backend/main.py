...
evo2_image = (
    ...
    .pip_install(
        "setuptools>=42","wheel","ninja","requests"
    )
    .env({
        "CC": "/usr/bin/gcc",
        "CXX": "/usr/bin/g++",
    ...
    .run_commands(
        "git clone --recurse-submodules -b readme-ft https://github.com/ArcInstitute/evo2.git && "
        "sed -i \"s|return list(np.fromstring(text, dtype=np.uint8))|return list(np.frombuffer(text.encode('utf-8'), dtype=np.uint8))|g\" evo2/vortex/vortex/model/tokenizer.py && "
        "cd evo2 && pip install ."
    )
...
def run_brca1_analysis():
    ...
    auroc = roc_auc_score(y_true, -brcgetGenomeChromosomesa1_subset['evo2_delta_score'])
